...
public class RileyAbelButton {
	private Button rileyAbel;
	
	public RileyAbelButton() {
		rileyAbel = new Button();
		
		Image rileyAbelImg = new Image("Riley AbelTransparent.png");
		ImageView rileyAbelView = new ImageView(rileyAbelImg);
		rileyAbelView.setFitHeight(Main.height * 0.05);
		rileyAbelView.setPreserveRatio(true);
		rileyAbel.setPrefHeight(Main.height * 0.05);
		rileyAbel.setPrefWidth(rileyAbelView.getFitWidth());
		rileyAbel.setGraphic(rileyAbelView);
		rileyAbel.setBackground(null);
		
		rileyAbel.setOnMouseEntered(new EventHandler<MouseEvent>() {
		...
		rileyAbel.setOnMouseExited(new EventHandler<MouseEvent>() {
		...
		rileyAbel.setOnMouseClicked(new EventHandler<MouseEvent>() {

			@Override
			public void handle(MouseEvent arg0) {
				BackgroundImage myBI = new BackgroundImage(new Image("RileySelection.png", 0, 0, false, true), BackgroundRepeat.NO_REPEAT, BackgroundRepeat.NO_REPEAT, BackgroundPosition.DEFAULT, new BackgroundSize(1.0, 1.0, true, true, false, false));
				CharacterSelection.root.setBackground(new Background(myBI));
				CharacterSelection.selected = Game.availableHeroes.get(3);
			}
