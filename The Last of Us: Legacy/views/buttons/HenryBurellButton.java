...
public class HenryBurellButton {
	private Button henryBurell;
	
	public HenryBurellButton() {
		henryBurell = new Button();
		
		Image henryBurellImg = new Image("Henry BurellTransparent.png");
		ImageView henryBurellView = new ImageView(henryBurellImg);
		henryBurellView.setFitHeight(Main.height * 0.05);
		henryBurellView.setPreserveRatio(true);
		henryBurell.setPrefHeight(Main.height * 0.05);
		henryBurell.setPrefWidth(henryBurellView.getFitWidth());
		henryBurell.setGraphic(henryBurellView);
		henryBurell.setBackground(null);
		
		henryBurell.setOnMouseEntered(new EventHandler<MouseEvent>() {
		...
		henryBurell.setOnMouseExited(new EventHandler<MouseEvent>() {
		...
		henryBurell.setOnMouseClicked(new EventHandler<MouseEvent>() {

			@Override
			public void handle(MouseEvent arg0) {
				BackgroundImage myBI = new BackgroundImage(new Image("HenrySelection.png", 0, 0, false, true), BackgroundRepeat.NO_REPEAT, BackgroundRepeat.NO_REPEAT, BackgroundPosition.DEFAULT, new BackgroundSize(1.0, 1.0, true, true, false, false));
				CharacterSelection.root.setBackground(new Background(myBI));
				CharacterSelection.selected = Game.availableHeroes.get(7);
			}
